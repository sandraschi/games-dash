...
def migrate_questions():
    ...
    try:
            ...
            for question_type, questions in types.items():
                for question in questions:
                    ...
                    cursor = db.execute('''
                        ...
                        VALUES (?, ?, ?, ?, ?)
                    ''', [level, question_type, question['question'], question['correct'], 1])

                    question_id = cursor.lastrowid

                    # Insert options
                    for letter, option_text in question['options'].items():
                        explanation = question['explanations'].get(letter, f"Option {letter} explanation")
                        db.execute('''
        ...
        db.commit()
        print(f"Successfully migrated {sum(len(types[qtype]) for types in questions_data.values() for qtype in types)} questions to database")

...
# Bundle .env.example (NOT .env - dev .env has personal credentials). The installer
# ships the template; the app copies it to %LOCALAPPDATA% on first run for user config.
$envExample = "$Root\.env.example"
if (Test-Path $envExample) {
    Copy-Item $envExample "$ResourceDir\.env.example" -Force
    Write-Host "  Bundled .env.example (template)" -ForegroundColor Green
} else {

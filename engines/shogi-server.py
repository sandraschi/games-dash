...
async def start_background_tasks(app):
    ...
    try:
        # Find YaneuraOu executable (env var overrides auto-detect)
        env_path = os.environ.get("YANEURAOU_PATH")
        if env_path:
            yaneuraou_exe = env_path
            print(f"[OK] Found YaneuraOu (from env): {yaneuraou_exe}")
        else:
            yaneuraou_paths = [
...
def main():
    ...
    try:
        ...
        web.run_app(app, host=os.environ.get("YANEURAOU_HOST", "127.0.0.1"), port=port)
    except OSError as e:
        if e.errno == 10048:

...
function updateCameraPosition() {
    // Adjust camera distance based on puzzle type and size to ensure padding
    let distance = 4;

    if (puzzleType.startsWith('cube-')) {
        distance = 4 + (cubeSize - 3) * 1.0;
    } else if (puzzleType === 'megaminx') {
        distance = 6.5; // Megaminx is larger
    } else if (puzzleType === 'pyraminx') {
        distance = 5.5; // Pyraminx is medium-sized
    } else if (puzzleType === 'skewb') {
        distance = 4.5; // Skewb is similar to 3x3
    }

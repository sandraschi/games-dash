...
# CRITICAL: If in stdio mode, wrap stdout immediately to suppress initialization noise
if _is_stdio_mode:
...
@mcp.tool()
async def help(topic: str = "overview") -> dict[str, Any]:
    """
    Get comprehensive help and documentation for Games MCP tools.

    Args:
        topic: The topic to get help for (overview, game_ops, analysis, sampling, etc.)
    """
    return help_sys.get_help(topic)
...
async def intelligent_game_analysis(
    ...
    See help(topic="sampling") for detailed documentation on autonomous orchestration.
    """
    try:
        # Get position
        if position:
        ...
        return {
            "success": False,
            "error": f"Analysis orchestration failed: {e!s}",
            ...
            "alternative_solutions": [
                "Use get_ai_move for basic analysis",
                "Use analyze_position_detailed for tactical analysis",
...
async def strategic_game_session(
    ...
    See help(topic="sampling") for session goals and adaptive learning details.
    """
    try:
        ...
        return {
            "success": False,
            "error": f"Learning session orchestration failed: {e!s}",
...
async def adaptive_game_coaching(
    ...
    See help(topic="sampling") for curriculum design and adaptive coaching details.
    """
    try:
        ...
        return {
            "success": False,
            "error": f"Coaching program design failed: {e!s}",
...
async def sampling_capabilities_status() -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"Cannot retrieve sampling status: {e!s}",
...
async def make_move(
    ...
    """
    Record a move in a correspondence game with enhanced AI analysis.

    See help(topic="game_ops") for notation examples and correspondence play details.
    """
    try:
        ...
        return {
            "success": False,
            "error": f"Failed to record move: {e!s}",
            ...
            "recovery_options": [
                ...
                "Use new_game to create game first",
            ...
            "alternative_solutions": [
                ...
                "Use analyze_position to analyze current position instead",
...
async def get_ai_move(
    ...
    See help(topic="game_ops") for difficulty levels and depth parameters.
    """
    try:
        logger.debug(
                ...
                if response.status == 200:
                    ...
                    return {
                        ...
                        "recommendations": [
                            ...
                            "Get deeper analysis with analyze_position_detailed(depth=20)",
        ...
        return {
            "success": False,
            "error": f"Cannot connect to {game_type} engine: {e!s}",
        ...
        return {
            "success": False,
            "error": f"Unexpected error getting AI move: {e!s}",
...
async def analyze_position(
    ...
    See help(topic="analysis") for evaluation metrics and engine details.
    """
...
async def get_game_state(game_id: str) -> dict[str, Any]:
    ...
    See help(topic="game_ops") for state details and status indicators.
    """
    try:
...
async def new_game(
    ...
    See help(topic="game_ops") for game types and ID generation.
    """
    import uuid
...
async def create_tournament(
    ...
    See help(topic="tournaments") for time controls and player management.
    """
    try:
...
async def register_for_tournament(tournament_id: str, player_id: str) -> dict[str, Any]:
    ...
    See help(topic="tournaments") for registration rules.
    """
    try:

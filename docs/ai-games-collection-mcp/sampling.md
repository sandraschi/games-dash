...
The AI Games Collection MCP server leverages advanced LLM sampling to autonomously manage complex game analysis and learning workflows.
...
All sampling tools use `ctx.sample()` with a rich `tools` parameter, enabling the LLM to call back into the AI Games Collection MCP server to perform atomic operations like `get_ai_move` or `find_tactical_motifs`.

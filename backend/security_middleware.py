...
class TokenBucketRateLimiter:
    ...
    def __init__(self, tokens_per_minute: int, burst_size: int, window_seconds: int = 60):
...
def log_request(request: web.Request, response_status: int, user_id: Optional[str] = None):
    ...
    if response_status >= 500:
        security_logger.error(f"Request failed: {request.method} {request.path} from {ip} - Status {response_status}")
    elif response_status >= 400:
        security_logger.warning(f"Request error: {request.method} {request.path} from {ip} - Status {response_status}")
    else:
        security_logger.debug(f"Request: {request.method} {request.path} from {ip} - Status {response_status}")
...
async def security_middleware(request: web.Request, handler):
    ...
    if AUTH_ENABLED:
        ...
        if not api_key:
            security_logger.warning(f"Authentication required but no API key provided from {client_ip}")
            log_request(request, 401, None)
            return web.json_response(
                {"success": False, "error": "Authentication required", "message": "API key required"},
                status=401
            )
        ...
        if not valid:
            ...
            security_logger.debug(f"Authenticated request from {client_ip} (user: {user_id})")
    else:
        user_id = "anonymous"

...
def vocab_to_question(vocab: dict, wrong_options: list[str]) -> dict:
    ...
    correct_letter = letters[options.index(correct)] if correct in options else letters[0]
    return {
        "question": f"「{expr}」の意味は次のうちどれですか。",
        "options": opts,
        "correct": correct_letter,
        "explanations": {k: f"「{expr}」は{opts[k]}の意味です。" if k == correct_letter else "" for k in opts},
    }

# 🚀 AI Games Collection MCP Server Deployment Guide

This guide covers deploying the AI Games Collection MCP server with **Streamable HTTP Transport** for maximum scalability and resilience.
...
```json
{
  ...
  "builds": [
    {
      "src": "src/ai_games_collection_mcp/mcp_server.py",
      "use": "@vercel/python"
    }
  ],
  "routes": [
    {
      "src": "/(.*)",
      "dest": "src/ai_games_collection_mcp/mcp_server.py"
    }
  ],
  "env": {
...
2. **Create function file** (`netlify/functions/ai-games-collection-mcp.py`):
```python
import sys
import os
sys.path.append(os.path.dirname(os.path.dirname(__file__)))

from src.ai_games_collection_mcp.mcp_server import main

def handler(event, context):
    # Netlify function handler for AI Games Collection MCP
    # Note: Requires custom MCP client integration
    return {
        'statusCode': 200,
        'body': 'AI Games Collection MCP Server - Use MCP client to connect'
    }
```
...
```bash
ai-games-collection-mcp --transport streamable-http --cors-origins "https://claude.ai,https://cursor.sh"
```
...
```javascript
// workers/ai-games-collection-mcp.js
import { handleRequest } from './ai-games-collection-mcp-server';
...
# US East: ai-games-collection-mcp-us-east.vercel.app
# EU West: ai-games-collection-mcp-eu-west.vercel.app
# Asia Pacific: ai-games-collection-mcp-apac.vercel.app
...
mcp = FastMCP("ai-games-collection-mcp", auth=auth)
```
...
```bash
# Test locally first
ai-games-collection-mcp --transport streamable-http --port 8000
...
```yaml
name: Deploy AI Games Collection MCP
...
**🎉 Your AI Games Collection MCP server is now ready for global, serverless deployment!**

...
logger = logging.getLogger("games_mcp.help")


class GamesHelpSystem:
    """
    Manages loading and serving documentation for the Games MCP server.
    """

    def __init__(self, docs_dir: str | None = None):
        # Default to docs/games_mcp relative to the project root
        if docs_dir is None:
            # Assume we are in src/games_mcp/ and project root is two levels up
            base_dir = Path(__file__).parent.parent.parent
            self.docs_dir = base_dir / "docs" / "games_mcp"
        else:

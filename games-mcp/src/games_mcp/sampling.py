...
class SamplingOrchestrator:
    ...
    async def orchestrate_analysis(
        ...
        try:
            ...
            self.performance_metrics["tool_calls_orchestrated"] += len(result.get("tool_calls", []))

            # Update average time
            total_time = self.performance_metrics["average_orchestration_time"] * (self.performance_metrics["orchestrations_completed"] - 1)
            self.performance_metrics["average_orchestration_time"] = (total_time + execution_time) / self.performance_metrics["orchestrations_completed"]

            return {
                "iterations": max_iterations,
                "execution_time": execution_time,
                "findings": result.get("content", "").split("\n"),
                "tools_used": [call.get("name") for call in result.get("tool_calls", [])],
            ...
            self.performance_metrics["error_rate"] = (
                (self.performance_metrics["orchestrations_attempted"] - self.performance_metrics["orchestrations_completed"])
    ...
    async def get_capabilities_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "features": [
                ...
                "adaptive_difficulty",
                "progress_tracking"
            ] if self.sampling_available else []
        }
    ...
    def _extract_recommendations(self, result: Dict[str, Any]) -> List[str]:
        ...
        for line in lines:
            line = line.strip()
            if any(keyword in line.lower() for keyword in ["recommend", "suggest", "consider", "try"]):
    ...
    def _assess_confidence(self, result: Dict[str, Any]) -> str:
        ...
        if len(tool_calls) >= 3 and content_length > 500:
            return "high"
        elif len(tool_calls) >= 2 and content_length > 200:
            return "medium"
        else:
            return "low"
    ...
    def _extract_concepts(self, result: Dict[str, Any]) -> List[str]:
        ...
        concept_keywords = [
            "tactics", "strategy", "endgame", "opening", "pawn structure",
            "piece coordination", "king safety", "development", "initiative",
            "calculation", "pattern recognition", "prophylaxis"
        ]

# Games MCP Server Testing Guide
...
python -m games_mcp.mcp_server 2>&1 | od -c | head -20

# Test 3: Validate JSON-RPC output
python -m games_mcp.mcp_server < test_input.json | python -m json.tool
```
...
**Fix**: Run `pip install -e .` in games-mcp directory
...
python -m games_mcp.mcp_server 2>&1 | Tee-Object -FilePath mcp_debug.log
```

### Validate JSON-RPC Messages
```powershell
# Capture and validate
python -m games_mcp.mcp_server < input.json | python -m json.tool
```
